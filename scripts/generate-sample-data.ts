...
async function main() {
  ...
  console.log('Generating sample data for the last 3 months...');
  ...
  for (let month = 0; month < 3; month++) {
    ...
    // Generate purchase orders (1-2 per month)
    ...
    for (let i = 0; i < purchaseOrdersCount; i++) {
      ...
      const orderNumber = `PO-${monthDate.getFullYear()}-${String(monthDate.getMonth() + 1).padStart(2, '0')}-${String(i + 1).padStart(3, '0')}-${Date.now()}`;
      ...
      if (existingPurchaseOrder) {
        console.log(`  Purchase order ${orderNumber} already exists, skipping...`);
        continue;
      }
    ...
    // Generate requests (3-8 per month)
    const requestsCount = Math.floor(Math.random() * 6) + 3;
    for (let i = 0; i < requestsCount; i++) {
      ...
      const existingRequest = await prisma.request.findFirst({
        where: {
          ...
          createdAt: {
            gte: startOfMonth(requestDate),
            lte: endOfMonth(requestDate)
          }
        }
      });
      
      if (existingRequest) {
        console.log(`  Request "${title}" for user ${requester.id} already exists, skipping...`);
        continue;
      }
    ...
    // Generate stock movements (5-15 per month)
    const movementsCount = Math.floor(Math.random() * 11) + 5;
    for (let i = 0; i < movementsCount; i++) {
      ...
      // Create a unique reference for the stock movement
      const reference = `MOV-${monthDate.getFullYear()}-${String(monthDate.getMonth() + 1).padStart(2, '0')}-${Date.now()}-${i}`;
      ...
      if (existingMovement) {
        console.log(`  Stock movement ${reference} already exists, skipping...`);
        continue;
      }
  ...
  console.log('📊 Summary:');
  console.log(`  - 3 months of data generated`);
