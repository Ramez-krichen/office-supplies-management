...
async function debugDepartmentCodes() {
  ...
  try {
    ...
    console.log(`📊 Found ${userDepartmentData.length} users with department assignments (showing first 20):`);
    ...
    Object.keys(groupedByDept).sort().forEach(deptName => {
      ...
      users.slice(0, 3).forEach(user => { // Show only first 3 users per department
    ...
    if (usersWithDeptIdButNoMatch.length > 0) {
      ...
      usersWithDeptIdButNoMatch.slice(0, 5).forEach(item => {
        console.log(`      - ${item.user}: departmentId "${item.departmentId}" (dept name: "${item.departmentName}")`);
      });
      if (usersWithDeptIdButNoMatch.length > 5) {
        console.log(`      ... and ${usersWithDeptIdButNoMatch.length - 5} more`);
      }
    }
